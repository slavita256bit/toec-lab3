...
#lab-figure(
  ...
  vector-diagram(
    ...
    currents: (
      // Увеличиваем ток в 100 раз для наглядности (т.к. он в амперах и очень мал)
...
#lab-figure(
  ...
  vector-diagram(
    ...
    voltages: (
      (val: U, label: $dot(U)$, anchor: "north-west"),
    ),

    currents: (
      // Масштаб 200, чтобы токи (~0.03А) выглядели как векторы длиной ~6 единиц
      // I1 - активный (по фазе с U)
      (val: I1_p_calc.val, scale: 200, label: $dot(I)_1$, anchor: "north"),
      // I2 - емкостной (идет вверх)
      (val: I2_p_calc.val, scale: 200, label: $dot(I)_2$, anchor: "east"),
      // I3 - индуктивный с потерями (идет вниз и вправо)
      (val: I3_p_calc.val, scale: 200, label: $dot(I)_3$, anchor: "south-west"),
    ),
...
// --- Вычисляем координаты точек для топографической диаграммы ---
#let i_sc = 400 // Масштаб для токов
#let p0 = (0, 0)
// Токи
#let p_i2 = (to-rect(I2_m).re * i_sc, to-rect(I2_m).im * i_sc)
#let p_i1 = (to-rect(I1_m).re * i_sc, to-rect(I1_m).im * i_sc)
// Напряжения
#let p_u2 = (to-rect(U2_m).re, to-rect(U2_m).im)
#let p_u = (to-rect(U).re, to-rect(U).im)
#let p_u3 = (to-rect(U3_m).re, to-rect(U3_m).im)

#lab-figure(
  caption: [Топографическая диаграмма напряжений и векторная диаграмма токов разветвленной цепи],
  vector-diagram(
    // Отключаем автоматическое связывание, мы зададим start/end вручную
    ...
    voltages: (
      ...
      (start: p0, end: p_u, label: $dot(U)$, anchor: "south"),

      // Ветвь 3: U3 + UC = U2
      (start: p0, end: p_u3, label: $dot(U)_3$, anchor: "west"),
      (start: p_u3, end: p_u2, label: $dot(U)_C$, anchor: "north-east"),
    ),

    currents: (
      // Сложение токов: I2 + I3 = I1
      (start: p0, end: p_i2, label: $dot(I)_2$, anchor: "north-west"),
      (start: p_i2, end: p_i1, label: $dot(I)_3$, anchor: "south-east"),
      // Суммарный ток I1
      (start: p0, end: p_i1, label: $dot(I)_1$, anchor: "west"),
    ),
